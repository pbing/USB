...
: token-setup/get-descriptor/device-to-host ( -- )
    device?         descriptor-index    0=   and  if  device-descriptor   dup c@ wLength @ min  data-packets ( data) zlp ( status)  exit then
    configuration?  descriptor-index    0=   and  if  configuration-descriptor  dup h# 2 +  @ ( wTotalLength) wLength @ min  data-packets ( data) zlp ( status)  exit then
    string?         descriptor-index    0=   and  if  string-descriptor0  dup c@ wLength @ min  data-packets ( data) zlp ( status)  exit then
    string?         descriptor-index h# 1 =  and  if  string-descriptor1  dup c@ wLength @ min  data-packets ( data) zlp ( status)  exit then
    string?         descriptor-index h# 2 =  and  if  string-descriptor2  dup c@ wLength @ min  data-packets ( data) zlp ( status)  exit then
    handshake-stall ;
